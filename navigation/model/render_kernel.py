...
def rkernel(queries, queries_rot, bases, bases_nvec, bases_value, cfg):
    ...
    bases_codebook_rot = bases_value[..., : G * D].reshape(N, B, G, D)#前G*D维特征向量用于rot
    bases_codebook_dist = bases_value[..., G * D :].reshape(N, B, H, D)#后H*D维特征向量用于dist
    ...
    if H > 0:
        ...
        anchor_closest_idx = anchor_dist.abs().min(dim=-1, keepdim=True)[1]  # N,Q,B,1，找一个采样点与一个地图上的点之间的距离属于哪个范围
    ...
    outangle = torch.atan2(vec_qb[..., 1:2], vec_qb[..., 0:1])  # N,Q,B,1 #顺时针为正方向，x轴为0°
    if queries_rot is not None:
        outangle += np.pi * 0.5 - queries_rot.reshape(
            N, Q, 1, 1
        )  # clock-wise, front zero degree, add offsets，以y轴负半轴顺时针旋转queries_rot后作为0°起始位置
    else:
    ...
    for i in range(Vr):
        ...
        visible_base_idx.append(amin)  # N,Q,1，circular feat的最左边（首位）对应着机器人的朝向
...
#############
    ...
    # distance_feat=dist_qb[i0_NQVr, i1_NQVr,visible_base_idx.reshape(-1),:].reshape(N, Q, Vr, 1)
    # angle_feat=inangle[i0_NQVr, i1_NQVr,visible_base_idx.reshape(-1)].reshape(N, Q, Vr)
    # print('circular_feat:',angle_feat)
#############
    ...
    if V > 0:
        ...
        ring_feat = F.pad(ring_feat, (pad_size, pad_size), mode="circular")#pad之后可以pool到V的大小
