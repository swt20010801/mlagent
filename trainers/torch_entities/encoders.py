...
class SimpleVisualEncoder(nn.Module):
    ...
    def forward(self, visual_obs: torch.Tensor) -> torch.Tensor:
        if not exporting_to_onnx.is_exporting():
            visual_obs = visual_obs.permute([0, 3, 1, 2])
        # pic_show=visual_obs[0].permute([1,2,0]).cpu().numpy()*255.
        # cv2.imwrite("vis.png",pic_show)
        hidden = self.conv_layers(visual_obs)
        hidden = hidden.reshape(-1, self.final_flat)
        return self.dense(hidden)
...
class ResNetVisualEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        resnet_backbone=resnet50(weights=models.ResNet50_Weights.DEFAULT)
        layers = [x for x in resnet_backbone.children()]
        self.sequential=nn.Sequential(*layers[:-2])

        self.conv=nn.Conv2d(2048,128,kernel_size=3,padding=1)

        out_shape=self.conv(self.sequential(torch.zeros((1,initial_channels,height,width)))).shape
        self.final_flat_size = out_shape[1]*out_shape[2]*out_shape[3]
