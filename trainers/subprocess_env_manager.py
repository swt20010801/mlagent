...
class SubprocessEnvManager(EnvManager):
    ...
    def _queue_steps(self,action_id:int) -> dict:
        for env_worker in self.env_workers:
            if not env_worker.waiting:
                ...
                run_out = {}
                action=AgentAction(None,[torch.ones((1,1))*action_id])

                run_out["env_action"] = action.to_action_tuple()
                run_out["entropy"]=torch.zeros((1))
                run_out["log_probs"]=ActionLogProbs(None,[torch.zeros((1))],[torch.zeros((1,3))])
                run_out["action"] = action.to_action_tuple()
                action_info={'RL_Agent?team=0':ActionInfo(
                            action=run_out.get("action"),
                            env_action=run_out.get("env_action"),
                            outputs=run_out,
                            agent_ids=[0],
                        )}
                env_worker.send(EnvironmentCommand.STEP, action_info)
                env_worker.waiting = True
                return env_worker.previous_step
    ...
    def _step(self,action_id:int) -> List[EnvironmentStep]:
        # Queue steps for any workers which aren't in the "waiting" state.
        previous_step=self._queue_steps(action_id=action_id)
        ...
        return step_infos,previous_step
