...
class TrainerController:
    ...
    def start_learning(self, env_manager: EnvManager) -> None:
        ...
        try:
            ...
            while self._not_done_training():
                # n_steps,FF = self.advance(env_manager,FF)
                # for _ in range(n_steps):
                #     self.reset_env_if_ready(env_manager)
                new_step_infos,previous_step = env_manager.get_steps(action_id=1)
                obs=previous_step.current_all_step_result['RL_Agent?team=0'][0].obs

                device=torch.device("cuda")
                if_new_episode=obs[4][0][0]
                now_position_x=obs[4][0][1]
                now_position_y=obs[4][0][2]
                now_rotation  =obs[4][0][3]/180*np.pi
    ...
    @timed
    def advance(self, env_manager: EnvManager,FF:dict=None) -> int:
        # Get steps
        with hierarchical_timer("env_step"):
            new_step_infos,FF = env_manager.get_steps(FF)
